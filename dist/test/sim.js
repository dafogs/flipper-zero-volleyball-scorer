...
function makeView(extraContracts) {
    ...
    for (const name of extraContracts)
        v[name] = contract();
    return v;
}
...
function fire(c, item) {
    const s = subs[c.__id];
    if (!s)
        throw new Error("no subscriber for contract " + c.__id);
...
for (let i = 0; i < 23; i++)
    fire(scoreInput, "left"); // A: 24
for (let i = 0; i < 22; i++)
    fire(scoreInput, "right"); // B: 23
...
// switch to B, give B the point it should have had: +1 -> 4
fire2("center");
ok(sc.props.header.indexOf("[4 B]") >= 0, "B selected");
fire2("right");
ok(sc.props.header.indexOf("[5 B]") >= 0, "B incremented to 5");
...
fire(navigation);
fire(currentView.chosen, 1); // Adjust, A selected
fire2("right"); // A 25
